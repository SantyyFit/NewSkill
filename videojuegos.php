...
<body class="videojuegos-body">
    <header class="header-Relaciones">
        <p class="videojuegos-title">Videojuegos</p>
    </header>

    <div class="publicaciones-container">
        <div class="publicar-contenedor">
            <div class="publicar-box">
                <a href="crearPost.php?user=<?= $_GET['user']; ?>&i=<?= $_GET['i']; ?>&or=videojuegos"
                    class="publicar-link">
        ...
        function mostrarComentarios($pdo, $idPublicacion, $idPadre = null, $margen = 0)
        {
            ...
            while ($coment = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                echo '<form action="comentar.php?user=' . $_GET["user"] . '&i=' . $_GET["i"] . '&or=videojuegos" method="POST">';
        ...
        <?php foreach ($posts as $post): ?>
            <div class="post-card" id="<?= $post['idPost']; ?>">
                <div class="user-info">
                    ...
                    <div>
                        <h3 class="username">
                            <a href="perfilUsuario.php?user=<?= $_GET['user'] ?>&UsuarioB=<?= $post['usuario_nombre'] ?>&idUsuarioB=<?= $post['idusuario'] ?>&i=<?= $_GET['i'] ?>"
                                class="username-link">
                ...
                // Obtener las reacciones para la publicación actual
...
");
                ...
                <div class="reacciones-contenedor">
                    <?php foreach ($tiposReaccion as $tipo => $emoji):
                        ...
                        $enlace = "reaccionar.php?post=" . $post['idPost'] . "&tipo=" . urlencode($tipo) . "&user=" . urlencode($_GET['user']) . "&i=" . urlencode($_GET['i']);
                    ?>
                ...
                <button onclick="toggleComentarios(<?= $post['idPost'] ?>)" class="btn-ver-comentarios">💬 Ver
                    comentarios</button>
                ...
                <div id="comentarios-<?= $post['idPost'] ?>" class="comentarios-contenedor">
                    ...
                    <form action="comentar.php?user=<?= $_GET['user'] ?>&i=<?= $_GET['i'] ?>&or=videojuegos" method="POST"
                        ...
                        <textarea name="comentario" rows="2" placeholder="Escribe tu comentario..." required
                            class="textarea-comentario"></textarea><br>
